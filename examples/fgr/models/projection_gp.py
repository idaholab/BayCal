...
def initialize(self, runInfo, inputs):
  ...
  self.ensembleRom = list(GaussianProcessRegressor(kernel=kernel, n_restarts_optimizer=20) for _ in range(self.romSize))
  # Fit to data using Maximum Likelihood Estimation of the parameters
  for i in range(self.romSize):
    self.ensembleRom[i].fit(inpData, outDiffR[i,:])

  self.time = np.asarray([0,1])
